/**
 * @file DaphneV3ControllerModule.cpp
 *
 * Implementation of the DAQModule to control the Daphne mezzanine board
 *
...
void
DaphneV3ControllerModule::init(std::shared_ptr<appfwk::ConfigurationManager> cfg)
{
...
void
DaphneV3ControllerModule::do_conf(const CommandData_t&)
{
  ...
  create_interface(board_conf->get_address(), general_conf->get_timeout());
...
void
DaphneV3ControllerModule::do_start(const CommandData_t&)
{
  ;
}
void
DaphneV3ControllerModule::do_scrap(const CommandData_t&)
{
...
void
DaphneV3ControllerModule::configure_analog_chain(bool initial_config)
{

  auto general_conf = m_module_config->get_daphne_conf();
  auto board_conf = initial_config ? m_module_config->get_board_conf() : general_conf->get_default_v3_settings();
  ...
  auto response = m_iface.load()->send<daphne::ConfigureResponse>(
    req.SerializeAsString(), daphne::MT2_CONFIGURE_FE_REQ, daphne::MT2_CONFIGURE_FE_RESP);
...
void
DaphneV3ControllerModule::create_interface(const std::string& address, std::chrono::milliseconds timeout)
{
...
void
DaphneV3ControllerModule::validate_configuration(const appmodel::DaphneV2BoardConf& c) const
{
...
DaphneV3ControllerModule::generate_opmon_data()
{
  ...
  auto response = m_iface.load()->send<daphne::ReadTriggerCountersResponse>(
    req.SerializeAsString(), daphne::MT2_READ_TRIGGER_COUNTERS_REQ, daphne::MT2_READ_TRIGGER_COUNTERS_RESP);
  ...
  for (const auto& c : snapshots) {
    ...
    if (c.threshold() == def_threshold && c.record_count() == 0 && c.busy_count() == 0 && c.full_count() == 0)
      continue;
  ...
  auto info_response = m_iface.load()->send<daphne::GeneralInfo>(
    "", daphne::MT2_READ_GENERAL_INFO_REQ, daphne::MT2_READ_GENERAL_INFO_RESP);

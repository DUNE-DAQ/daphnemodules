...
private:
  ...
  void create_interface(const std::string& address, std::chrono::milliseconds timeout);
  ...
  using const_channel_id_t = std::invoke_result<decltype(&daphne::ChannelConfig::id), daphne::ChannelConfig>::type;
  using ChannelId = std::remove_const<const_channel_id_t>::type;
  static const ChannelId s_max_channels = 40;

  using const_afe_id_t = std::invoke_result<decltype(&daphne::AFEConfig::id), daphne::AFEConfig>::type;

...
ERS_DECLARE_ISSUE(daphnemodules,
                  ...
                  "Board " << id << ": response from board was not parsed correctly for " << counter
                           << " times. Last Rseponse: " << response,
...
ERS_DECLARE_ISSUE(daphnemodules, InvalidBiasControl, bias << " bigger than 4095", ((uint64_t)bias))
...
ERS_DECLARE_ISSUE(daphnemodules,
                  InvalidLNAConf,
                  "AFE " << id << " has invalid LNA conf (reg52), clamp: " << clamp << ", gain: " << gain,
                  ((uint32_t)id)((uint32_t)clamp)((uint32_t)gain))

ERS_DECLARE_ISSUE(daphnemodules, InvalidThreshold, "Invalid threshold: " << threshold, ((uint32_t)threshold))
...
private:
  ...
  using const_metric_counter_t =
    std::invoke_result<decltype(&dunedaq::daphnemodules::opmon::ChannelInfo::total_triggers),

...
DaphneV3Interface::DaphneV3Interface(std::string address, std::string routing, std::chrono::milliseconds timeout)
  : m_context(1)
  ...
  , m_timeout(timeout)
{
  ...
  m_connection = string_values[2].matched ? fmt::format("tcp://{}", address)
...
ControlEnvelopeV2
DaphneV3Interface::send(std::string&& message, daphne::MessageTypeV2 type)
{
...
void
DaphneV3Interface::_send(std::string&& message, daphne::MessageTypeV2 type)
{
...
DaphneV3Interface::validate_connection()
{
  ...
  auto reply = send<TestRegResponse>(
    req.SerializeAsString(), MessageTypeV2::MT2_READ_TEST_REG_REQ, MessageTypeV2::MT2_READ_TEST_REG_RESP);

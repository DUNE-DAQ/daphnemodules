...
DaphneV2Interface::DaphneV2Interface(const char* ipaddr, int port, std::chrono::milliseconds timeout)
  : m_timeout(timeout)
{
...
bool
DaphneV2Interface::validate_connection() const
{
...
command_result
DaphneV2Interface::send_command_retry(std::string cmd, size_t retry) const
{
...
command_result
DaphneV2Interface::send_command_interruptible(std::string cmd, std::function<bool()> can_retry) const
{
...
command_result
DaphneV2Interface::send_command(std::string cmd) const
{
  ...
  for (size_t i = 0; i < (bytes.size() + 49) / 50; ++i) {
    std::vector<uint64_t> part(bytes.begin() + i * 50, bytes.begin() + std::min((i + 1) * 50, bytes.size()));
...
std::vector<uint64_t>
DaphneV2Interface::read(uint8_t command_id, uint64_t addr, uint8_t size) const
{
  ...
  if (result < 0)
    throw FailedSocketInteraction(ERS_HERE, "sendto");
  ...
  if (FD_ISSET(m_connection_id, &readfds)) {
    ...
    if (result <= 0)
      throw FailedSocketInteraction(ERS_HERE, "recvfrom");
...
void
DaphneV2Interface::write(uint8_t command_id, uint64_t addr, std::vector<uint64_t>&& data) const
{

...
ERS_DECLARE_ISSUE(daphnemodules, FailedSend, "Failed to send message of type " << type, ((std::string)type))
...
ERS_DECLARE_ISSUE(daphnemodules, EmptyPayload, "Empty payload received from " << connection, ((std::string)connection))
...
ERS_DECLARE_ISSUE(daphnemodules,
                  UnexpectedDirection,
                  "Message received with unexpeted properties. Version: " << version << ", direction "
                                                                          << ". Message: " << message,
...
public:
  DaphneV3Interface(std::string address, // it can contain the port or not
                    std::string routing, // this should be the name of the controller module
